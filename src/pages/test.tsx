...
import { addPendingJob, clearAllJob, clearPendingJob, pendingToOngoingJob, clearOngoingJob, ongoingToFulfilledJob, clearFulfilledJob, deleteFulfilledItemJob } from "../redux/todo/jobToDoSlice";
...
export const Test: FC<Props> = ({ title, data }) => {
    // return <>
    //     <div>Test Page: {title ?? "N/A"}! {data ?? "no data"}</div>
    //     <p>Text here</p>
    ...
    const pendingList = useSelector((state: RootState) => state.jobToDo.pendingValue);
    const ongoingList = useSelector((state: RootState) => state.jobToDo.ongoingValue);
    const fulfilledList = useSelector((state: RootState) => state.jobToDo.fulfilledValue);
    ...
    const submitForm = () => {
        dispatch(addPendingJob(currentItem));
    ...
    return <div style={{ minHeight: "100dvh" }}>
        ...
        <header className="p-4 border-b flex flex-row justify-between items-center">
            ...
            <div className="w-full">
                <p className="text-xl font-bold text-stone-700">Job To Do List ({pendingList.length + ongoingList.length + fulfilledList.length})</p>
                <p>Your guide</p>
            </div>
        </header>
            ...
            <div className="w-full lg:w-1/3 px-4 py-1">
                <div className="border border-stone-300 shadow-md rounded-md p-4">
                    <div className="flex flex-row justify-start">
                        ...
                        <button className="mx-2" onClick={() => dispatch(clearPendingJob())}>
                            <MdDelete />
                        </button>
                    </div>
                    <div className="py-4">
                        {
                            pendingList.map((item) => (
                                <div className="flex flex-row justify-between p-2">
                                    ...
                                    <button className="" onClick={() => dispatch(pendingToOngoingJob(item))}>
            ...
            <div className="w-full lg:w-1/3 px-4 py-1">
                <div className="border border-stone-300 shadow-md rounded-md p-4">
                    <div className="flex flex-row justify-start">
                        ...
                        <button className="mx-2" onClick={() => dispatch(clearOngoingJob())}>
                            <MdDelete />
                        </button>
                    </div>
                    <div className="py-4">
                        {
                            ongoingList.map((item) => (
                                <div className="flex flex-row justify-between p-2">
                                    ...
                                    <button className="" onClick={() => dispatch(ongoingToFulfilledJob(item))}>
                                        <FaArrowAltCircleRight />
                                    </button>
                                </div>
                            ))
            ...
            <div className="w-full lg:w-1/3 px-4 py-1">
                <div className="border border-stone-300 shadow-md rounded-md p-4">
                    <div className="flex flex-row justify-start">
                        ...
                        <button className="mx-2" onClick={() => dispatch(clearFulfilledJob())}>
                            <MdDelete />
                        </button>
                    </div>
                    <div className="py-4">
                        {
                            fulfilledList.map((item) => (
                                <div className="flex flex-row justify-between p-2">
                                    ...
                                    <button className="" onClick={() => dispatch(deleteFulfilledItemJob(item))}>
                                        <MdDelete />
                                    </button>
                                </div>
                            ))
